...
async function handleWarning(ctx, senderJid, senderId, senderLid, groupJid, groupDb) {
    ...
    await ctx.reply({
        text: tools.msg.info(`Warning ${currentWarnings}/${maxWarnings} for @${ctx.getId(senderId)}!`),
        mentions: [senderJid]
    });
    ...
    if (currentWarnings >= maxWarnings) {
        ...
        if (isBotAdmin) {
            await ctx.reply(tools.msg.info(`You have received ${maxWarnings} warnings and will be removed from the group!`));
            if (!config.system.restrict) await ctx.group().kick(senderJid);
            groupDb.warnings = warnings.filter(warning => warning.jid !== senderLid);
        } else {
            await ctx.reply(tools.msg.info(`${config.msg.botAdmin} Cannot remove user who reached ${maxWarnings} warnings.`));
        }
    }
...
module.exports = (bot) => {
    bot.ev.on(Events.MessagesUpsert, async (ctx) => {
        ...
        if (isGroup || isPrivate) {
                ...
                if (reportOwner && reportOwner.length > 0) {
                    ...
                    for (const ownerId of reportOwner) {
                        await ctx.replyWithJid(ownerId + Baileys.S_WHATSAPP_NET, {
                            text: tools.msg.info(`Account @${senderId} has been auto-banned due to ${formatter.inlineCode(`Anti Bug - ${analyze.reason}`)}, severity ${formatter.inlineCode(analyze.severity)}, threat type ${formatter.inlineCode(analyze.severity)}.`),
                            mentions: [senderJid]
                        });
            ...
            if (isCmd?.didyoumean)
                await ctx.reply({
                    text: tools.msg.info(`Did you mean ${formatter.inlineCode(isCmd.prefix + isCmd.didyoumean)}?`),
                    buttons: [{ text: "Yes, that's right!", id: `${isCmd.prefix + isCmd.didyoumean} ${isCmd.input}` }]
                });
            ...
            if (senderAfk.reason || senderAfk.timestamp) {
                ...
                if (timeElapsed > 3000) {
                    ...
                    await ctx.reply(tools.msg.info(`You have returned from AFK ${senderAfk.reason ? `with reason ${formatter.inlineCode(senderAfk.reason)}` : "without reason"} after ${timeago}.`));
                    delete senderDb.afk;
                    senderDb.save();
                }
            }
            ...
            if (isGroup) {
                ...
                if (afkMentions.length > 0) {
                    for (const afkMention of afkMentions) {
                        ...
                        if (mentionAfk.reason || mentionAfk.timestamp) {
                            ...
                            await ctx.reply({
                                text: tools.msg.info(`Don't disturb! @${ctx.getId(afkMention)} is AFK ${mentionAfk.reason ? `with reason ${formatter.inlineCode(mentionAfk.reason)}` : "without reason"} for ${timeago}.`),
                                mentions: [afkMention]
                            });
                        }
                    }
                ...
                if (!isCmd && !isOwner && !isAdmin) {
                    ...
                    if (groupDb?.option?.antigcsw) {
                        ...
                        if (checkMedia) {
                            await ctx.reply(tools.msg.info("Do not post group statuses!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antilink) {
                        if (msg.body && tools.cmd.isUrl(msg.body)) {
                            await ctx.reply(tools.msg.info("Do not send links!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antispam) {
                        ...
                        const senderSpam = spamData.find(spam => tools.cmd.areJidsSameUser(spam.jid, senderLid)) || {
                            jid: senderLid, count: 0, lastMessageTime: 0
                        };
                        ...
                        if (newCount > 5) {
                            await ctx.reply(tools.msg.info("Stop spamming!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antitagsw) {
                        ...
                        if (!!checkMedia) {
                            await ctx.reply(tools.msg.info("Do not tag the group in statuses!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antitoxic) {
                        ...
                        if (msg.body && toxicRegex.test(msg.body)) {
                            await ctx.reply(tools.msg.info("Do not use toxic language!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
