    ...
    code: async (ctx) => {
        if (ctx.args[0]?.toLowerCase() === "all") {
            ...
            try {
                ...
                return await ctx.reply(tools.msg.info(`Successfully menyetujui semua anggota (${allJids.length}).`));
            } catch (error) {
        ...
        if (!target.jid)
            return await ctx.reply(
                ...
                tools.msg.generateNotes([
                    `Ketik ${formatter.inlineCode(`${ctx.used.prefix + ctx.used.command} all`)} to approve all pending members.`
                ])
            );
        ...
        try {
            ...
            await ctx.reply(tools.msg.info("Successfully disetujui!"));
        } catch (error) {
