module.exports = {
    ...
    code: async (ctx) => {
        ...
        try {
            ...
            await ctx.groups.acceptInvite(urlCode).then(async (res) => {
                if (res)
                    await ctx.sendMessage(res, {
                        ...
                        caption: `>ᴗ< ${formatter.italic(`Halo! Saya adalah bot WhatsApp bernama ${config.bot.name}, dimiliki oleh ${config.owner.name}. Saya bisa melakukan banyak perintah, seperti membuat stiker, menggunakan AI untuk pekerjaan tertentu, dan beberapa perintah berguna lainnya. Saya di sini untuk menghibur dan menyenangkan You!`)}`,
            ...
            await ctx.reply(tools.msg.info("Successfully bergabung dengan grup!"));
        } catch (error) {
