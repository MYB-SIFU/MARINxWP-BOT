module.exports = {
    ...
    code: async (ctx) => {
        ...
        if (!input)
            return await ctx.reply(
                ...
                tools.msg.generateNotes([
                    "AI ini dapat melihat media.",
                    `Type ${formatter.inlineCode(`${ctx.used.prefix + ctx.used.command} reset`)} to reset the conversation history.`
                ])
            );
        ...
        if (input === "reset") {
            ...
            return await ctx.reply(tools.msg.info("Riwayat percakapan successfully direset!"));
        }
