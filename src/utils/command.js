...
function calculateDelay(totalTargets) {
    ...
    const delays = Array.from({ length: totalTargets }, () => {
    ...
    return { delay: delays, duration: totalDuration };
}
...
function getReportOwner() {
    ...
    if (config.owner.co && Array.isArray(config.owner.co))
        config.owner.co.forEach(co => { if (co.report) owners.push(co.id); });
...
async function handleError(ctx, error, useAxios = false, silent = false) {
    if (!silent) {
        ...
        if (reportOwner && reportOwner.length > 0) {
            const { delay } = calculateDelay(reportOwner.length);
            for (const ownerId of reportOwner) {
                await ctx.replyWithJid(ownerId + Baileys.S_WHATSAPP_NET, {
                    text: `${tools.msg.info(isGroup ? `An error occurred from group: @${groupJid}, by: @${senderId}` : `An error occurred from: @${senderId}`)}\n` +
                        formatter.monospace(errorText),
                    contextInfo: {
                        mentionedJid: [senderJid],
                        groupMentions: isGroup ? [{ groupJid, groupSubject }] : []
                    }
                });
    ...
    await ctx.reply(tools.msg.info(`An error occurred: ${error.message}`));
}
