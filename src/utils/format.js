...
function convertMsToDuration(ms, requestedParts = null) {
    ...
    const units = {
        years: { value: duration.years(), condition: duration.years() > 0 },
        months: { value: duration.months(), condition: duration.months() > 0 },
        weeks: { value: duration.weeks(), condition: duration.weeks() > 0 },
        days: { value: duration.days(), condition: duration.days() > 0 },
        hours: { value: duration.hours(), condition: duration.hours() > 0 },
        minutes: { value: duration.minutes(), condition: duration.minutes() > 0 },
        seconds: { value: duration.seconds(), condition: duration.seconds() > 0 },
        milliseconds: { value: duration.milliseconds(), condition: duration.milliseconds() > 0 }
    };
...
function formatSize(byteCount, withPerSecond = false) {
    ...
    while (size < 1 && index > 0) { size *= 1024; index--; }
    while (size >= 1024 && index < bytes.length - 1) { size /= 1024; index++; }

    return `${size.toFixed(2)} ${bytes[index]}${withPerSecond ? "/s" : ""}`;
}

function generateCmdExample(used, args) {
    if (!used || !args) return `${formatter.inlineCode("used")} or ${formatter.inlineCode("args")} must be provided!`;
    return `Example: ${formatter.inlineCode(`${used.prefix + used.command} ${args}`)}`;
}

function generateInstruction(actions, mediaTypes) {
    if (!actions || !mediaTypes || !Array.isArray(actions) || !Array.isArray(mediaTypes))
        return `${formatter.inlineCode("actions")} and ${formatter.inlineCode("mediaTypes")} must be arrays!`;

    const mediaTypesList = mediaTypes.length > 1
        ? `${mediaTypes.slice(0, -1).join(", ")} or ${mediaTypes[mediaTypes.length - 1]}`
        : mediaTypes[0];

    const actionMap = { send: "Send", reply: "Reply with" };
    const actionList = actions.map(a => actionMap[a] || a).join(actions.length > 1 ? " or " : "");
...
function generateNotes(notes) {
    if (!notes || !Array.isArray(notes)) return `${formatter.inlineCode("notes")} must be an array!`;
